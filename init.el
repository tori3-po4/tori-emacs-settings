...
;; 全般パフォーマンス
;;   ・bidi-inhibit-bpa:
;;       双方向テキストの段落解析 (BPA) は行が長くなるほど重い。
;;       抑止しても R2L 言語の表示自体には影響しない
;;       (CJK + 英字環境では実害なし)。長行ファイルの再描画が高速化。
;;   ・jit-lock-defer-time:
;;       タイピング/スクロール直後の同期着色を 0.05 秒だけ遅延し、
;;       操作中の小さなフリーズを抑える。入力が止まると着色が始まる。
;;   ・read-process-output-max:
;;       サブプロセス出力の読み取りバッファ拡大 (既定 4KB → 1MB)。
;;       magit / diff-hl / 外部コンパイラ等、長出力を扱う処理が高速化。
;; ============================================================
